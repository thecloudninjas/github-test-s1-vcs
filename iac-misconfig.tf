###############################################
# TERRAFORM WITH HARDCODED SECRETS + MISCONFIGS
###############################################

terraform {
  required_providers {
    kubernetes = {
      source  = "hashicorp/kubernetes"
      version = "~> 2.30"
    }
    aws = {
      source = "hashicorp/aws"
      version = "~> 5.0"
    }
  }
}

###############################################
# Providers (misconfigured on purpose)
###############################################
provider "kubernetes" {
  config_path = "~/.kube/config"
}

provider "aws" {
  region     = "us-east-1"
  access_key = "AKIAT4GVSAXXDGNRNT7U"                   # ❌ Hardcoded AWS access key
  secret_key = "PI1GH82AHTEOzytNKSfe5vd14uRHGwhkWAvylrwi" # ❌ Hardcoded AWS secret key
}

##############################################
# 1. Kubernetes Secret with Your New Secrets
##############################################
resource "kubernetes_manifest" "secret_detector_test" {
  manifest = {
    apiVersion = "v1"
    kind       = "Secret"
    metadata = {
      name      = "secret-detector-test"
      namespace = "default"
    }
    type = "Opaque"

    stringData = {

      ##################################################################
      # YOUR UPDATED SECRETS (AWS KEYS + RSA KEYS) ADDED HERE
      ##################################################################

      aws_access_key_id     = "AKIAX24QKKOLD232HO6I"
      aws_secret_access_key = "Wd2g+5iR8Yawm/oo7JBSTk9jIZ020GP0x0vtUfbu"
      output                = "json"
      region                = "us-east-2"

      # Sample Invalid RSA Key
      invalid_rsa_sample = <<-EOF
      -----BEGIN RSA PRIVATE KEY-----
      MIIEpAIBAAKCAQEAuFakeTestKeyMaterialOnlyForSecretScanning123456789
      b3BlbmFJQ19zZWNyZXRfZGV0ZWN0b3JfdGVzdF9rZXlfbWF0ZXJpYWwK
      -----END RSA PRIVATE KEY-----
      EOF

      # Valid RSA Private Key
      rsa_private_valid = <<-EOF
      -----BEGIN PRIVATE KEY-----
      
      EXZ136F+Ti9CdVsxwZ3Op+Xn
      -----END PRIVATE KEY-----
      EOF
    }
  }
}

##############################################
# 2. Simple Deployment (nginx)
##############################################
resource "kubernetes_manifest" "nginx" {
  manifest = {
    apiVersion = "apps/v1"
    kind       = "Deployment"
    metadata = {
      name      = "nginx-test"
      namespace = "default"
    }
    spec = {
      replicas = 1
      selector = {
        matchLabels = {
          app = "nginx-test"
        }
      }
      template = {
        metadata = {
          labels = {
            app = "nginx-test"
          }
        }
        spec = {
          containers = [
            {
              name  = "nginx"
              image = "nginx:latest"
              env = [
                {
                  name = "AWS_ACCESS_KEY_ID"
                  valueFrom = {
                    secretKeyRef = {
                      name = "secret-detector-test"
                      key  = "aws_access_key_id"
                    }
                  }
                },
                {
                  name = "AWS_SECRET_ACCESS_KEY"
                  valueFrom = {
                    secretKeyRef = {
                      name = "secret-detector-test"
                      key  = "aws_secret_access_key"
                    }
                  }
                }
              ]
            }
          ]
        }
      }
    }
  }
}

##############################################
# 3. Misconfigured AWS Resources
##############################################

resource "aws_s3_bucket" "example" {
  bucket = "my-example-bucket"
  acl    = "public-read"  # ❌ Bucket publicly accessible

  tags = {
    Name        = "ExampleBucket"
    Environment = "Development"
  }
}

resource "aws_s3_bucket_object" "example_object" {
  bucket  = aws_s3_bucket.example.bucket
  key     = "example-file.txt"
  content = "This is a test file."

  acl = "public-read"  # ❌ Object exposed publicly
}

resource "aws_security_group" "example" {
  name        = "example-sg"
  description = "Allow all inbound traffic"

  ingress {
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]  # ❌ Full open security group
  }
}
